...
function ImagePicker({ onImageSelect }) {
  ...
  // Fetch images from Unsplash and log the result
  const searchImages = async () => {
    ...
    try {
      ...
      const imageUrls = data.results.map((image) => image.urls.small); // Extract image URLs
      setFetchedImages(imageUrls); // Update state with image URLs
    } catch (error) {
